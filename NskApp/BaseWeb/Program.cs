using BaseWeb.Common.Extensions;
using BaseWeb.Core.Config;
using BaseWeb.Core.Cookie;
using BaseWeb.Core.Filter;
using BaseWeb.Core.Middleware;
...
builder.Services.AddAuthentication(CookieAuthenticationDefaults.AuthenticationScheme).AddCookie(options =>
{
    ...
    options.AccessDeniedPath = new PathString("/F90/D9004/Init");
});
...
builder.Services.AddMvc(options =>
{
    ...
    // ログインユーザをセッションに設定
    options.Filters.Add<LoginUserSettingBaseWebAttribute>();

    // ロギング
    options.Filters.Add<LoggingFilter>();

    // リクエストの有効性チェック
    options.Filters.Add<ValidRequestBaseWebAttribute>();

    /// システムロックチェック
    options.Filters.Add<ValidSystemLockBaseWebAttribute>();
...
app.UseEndpoints(endpoints =>
{
    ...
    _ = endpoints.MapControllerRoute(
        ...
        defaults: new { area = "F00", controller = "D0001", action = "Init" },
        dataTokens: new { area = "F00" }
    );
});
