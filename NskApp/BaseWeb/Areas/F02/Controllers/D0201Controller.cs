...
using BaseWeb.Areas.F02.Models.D0201;
...
namespace BaseWeb.Areas.F02.Controllers
{
    /// <summary>
    /// 一括帳票出力
    /// </summary>
    ...
    public class D0201Controller : CoreController
    {
        ...
        private List<D0201TableRecord> GetResult(D0201Model model, int offset = 0, int pageSize = 10)
        {
            ...
            return getJigyoDb<BaseAppContext>().Database.SqlQueryRaw<D0201TableRecord>(sql.ToString(), parameters.ToArray()).ToList();
        }
        #endregion
        ...
        private string SetDisplayJouken(D0201Model model, int joukenId)
        {
            ...
            var joukenList = getJigyoDb<BaseAppContext>().TReportJoukens.Where(t => t.JoukenId == joukenId && !t.JoukenNm.StartsWith("表示順") && !constJouken.Contains(t.JoukenNm)).OrderBy(t => t.SerialNumber);
