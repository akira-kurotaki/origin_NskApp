...
using BaseWeb.Areas.F01.Models.D0102;
...
namespace BaseWeb.Areas.F01.Controllers
{
    ...
    public class D0102Controller : CoreController
    {
        ...
        private bool HaveNogyoshaKengen(int nogyoshaId, string todofukenCd, string kumiaitoCd, string shishoCd, List<Shisho> shishoList)
        {
            ...
            var result = getJigyoDb<BaseAppContext>().Database.SqlQueryRaw<int>(sql.ToString(), parameters.ToArray()).ToList();
        ...
        private VNogyosha GetNogyoshaInfo(int nogyoshaId)
        {
            ...
            return getJigyoDb<BaseAppContext>().VNogyoshas
        ...
        private TKanyusha GetKanyushaInfo(int nogyoshaId, short? nendo)
        {
            ...
            return getJigyoDb<BaseAppContext>().TKanyushas
        ...
        private List<TKanyusha> GetKanyushaList(int nogyoshaId)
        {
            ...
            return getJigyoDb<BaseAppContext>().TKanyushas.Where(t => t.NogyoshaId == nogyoshaId).ToList();
        }
        ...
        private int InsertTReportJouken(string nogyoshaId, string nendo)
        {
            ...
            int joukenId = (int)DBUtil.NextSeqVal(getJigyoDb<BaseAppContext>(), "seq_jouken_id");
            ...
            using (var tx = getJigyoDb<BaseAppContext>().Database.BeginTransaction())
            {
                // 農業者ID
                ...
                getJigyoDb<BaseAppContext>().TReportJoukens.Add(jouken);
                logger.Info("帳票作成条件を条件テーブルに登録する(条件名：" + JoukenNameConst.JOUKEN_NOGYOSHA_ID + ")");
                getJigyoDb<BaseAppContext>().SaveChanges();
        ...
        private bool KanyushaCdChk(int? nogyoshaId, string kanyushaCd)
        {
            ...
            var cnt = getJigyoDb<BaseAppContext>().Database.SqlQueryRaw<int>(sql.ToString(), parameters.ToArray()).Single();
            if (1 <= cnt)
            {
        ...
        private bool Insert(int? nogyoshaId, D0102Model form, DateTime sysDate)
        {
            ...
            using (var tx = getJigyoDb<BaseAppContext>().Database.BeginTransaction())
            {
                ...
                getJigyoDb<BaseAppContext>().TKanyushas.Add(kanyusha);
        ...
        private bool Update(D0102Model form, TKanyusha kanyusha, DateTime sysDate)
        {
            using (var tx = getJigyoDb<BaseAppContext>().Database.BeginTransaction())
            {
                ...
                getJigyoDb<BaseAppContext>().Entry(kanyusha).State = EntityState.Modified;
