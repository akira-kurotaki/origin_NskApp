using BaseWeb.Areas.F00.Models.D0000;
...
namespace BaseWeb.Areas.F00.Controllers
{
    ...
    [Area("F00")]
    public class D0000Controller : CoreController
    {
        /// <summary>
        /// コンストラクタ
        /// </summary>
        /// <param name="viewEngine"></param>
        public D0000Controller(ICompositeViewEngine viewEngine) : base(viewEngine)
        {
        }
        ...
        public ActionResult Index()
        {
            if (ConfigUtil.Get(CoreConst.D0000_DISPLAY_FLAG) == "true")
            {
                ...
                // 簡易ログイン画面
                return View("D0000_Pre", GetInitModel());
            }
            else
            {
                return RedirectToAction("Init", "D9004", new { area = "F90" });
            }
        }
        ...
        private D0000Model GetInitModel()
        {
            D0000Model model = new D0000Model();
        ...
        /// <returns></returns>
        [HttpPost]
        public async Task<ActionResult> LoginAsync([Bind("UserId,Password")] D0000Model model)
        {
            if (ConfigUtil.Get(CoreConst.D0000_DISPLAY_FLAG) == "true")
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (mSyokuin == null)
                    {
                        ...
                        D0000Model d0000Model = GetInitModel();
                        d0000Model.UserId = model.UserId;
                        return View("D0000_Pre", d0000Model);
                    }
        ...
        private bool AuthUser(D0000Model model, MSyokuin mShokuin)
        {
            bool result = false;
            // ユーザIDが存在する
            // アカウントロックチェック
        ...
        /// </summary>
        [HttpPost]
        public async Task<ActionResult> DebugLoginAsync([Bind("UserId,TodofukenCd,KumiaitoCd,ShishoCd,UserKanriKengen,Name,NameKana,Tel,Mail,SystemRiyoKbn,ScreenSosaKengenId,ShishoGroupId,UserKanriKengen,ScreenMode")] D0000Model model)
        {
            // 認証クッキーの発行
        ...
        public ActionResult Todofuken([Bind("TodofukenCd,KumiaitoCd,ShishoCd"), FromBody] D0000Model model)
        {
            D0000Model returnModel = new D0000Model();
        ...
        /// </summary>
        /// <returns>ActionResult</returns>
        /// GET: F00/D0000/Kumiaito
        public ActionResult Kumiaito([Bind("TodofukenCd,KumiaitoCd,ShishoCd"), FromBody] D0000Model model)
        {
            D0000Model returnModel = new D0000Model();
            string kumiaitoNm = KumiaitoUtil.GetKumiaitoNm(model.TodofukenCd, model.KumiaitoCd);
            if (!string.IsNullOrEmpty(kumiaitoNm))
        ...
        public ActionResult Shisho([Bind("TodofukenCd,KumiaitoCd,ShishoCd"), FromBody] D0000Model model)
        {
            D0000Model returnModel = new D0000Model();
            string shishoNm = ShishoUtil.GetShishoNm(model.TodofukenCd, model.KumiaitoCd, model.ShishoCd);
            if (!string.IsNullOrEmpty(shishoNm))
